...
function chunkToLinesForWidth(words, width, measure) {
  const lines = [[]];
...
function drawUnderline(document, start, end) {
  document.save();
...
//export
function generatePDF(outputStream, scriptAst, givenOptions = {}) {
  const options = Object.assign(Object.assign({}, defaultOptions), givenOptions);
  ...
  doc.font(typeface.regular).text(`${scriptAst.metadata.credit || 'by'} ${scriptAst.metadata.author || scriptAst.metadata.authors}`);
  ...
  scriptAst.tokens.forEach((token) => {
    switch (token.type) {
      case Token.Character: {
        ...
        currentCharacterStartingHeight = doc.y;
        doc.font(typeface.regular).text(token.text, MARGIN, doc.y, { width: CHARACTER_WIDTH });
        currentCharacterLowestHeight = doc.y;
        doc.font(typeface.regular).text(characterCount, doc.page.width - MARGIN, currentCharacterStartingHeight, { width: MARGIN });
      ...
      case Token.Action: {
        doc.font(typeface.regular).text(token.text, MARGIN, currentCharacterStartingHeight + 10, {
      ...
      case Token.Parenthetical: {
        doc
          ...
          .text(token.text.toUpperCase().replace(/\n/g, ''), MARGIN + CHARACTER_WIDTH + MARGIN, currentCharacterStartingHeight, {
            width: DIALOGUE_WIDTH,
            continuous: true,
          });
        currentCharacterStartingHeight = doc.y;
      ...
      case Token.FX: {
        ...
        doc.font(typeface.bold).text(`${token.style}.`, MARGIN, currentCharacterStartingHeight, { width: doc.page.width - MARGIN * 2 });
        const lines = chunkToLinesForWidth(token.text.split(' '), DIALOGUE_WIDTH, doc.widthOfString.bind(doc)).map((m) => m.join(' '));
        doc.font(typeface.bold).text(lines[0], MARGIN + CHARACTER_WIDTH + MARGIN, currentCharacterStartingHeight, { width: DIALOGUE_WIDTH });
        drawUnderline(doc, MARGIN, doc.x + doc.widthOfString(lines[0]));
        doc.font(typeface.bold).text(characterCount, doc.page.width - MARGIN, currentCharacterStartingHeight, { width: MARGIN });
        characterCount = characterCount + 1;
        break;
      }
      case Token.Dialogue: {
        ...
        console.log(lines);
        lines.forEach((line) => {
          doc.font(typeface.regular).text(line.join(' '), MARGIN + CHARACTER_WIDTH + MARGIN, currentCharacterStartingHeight);
